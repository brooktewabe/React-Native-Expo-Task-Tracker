...
const Signup = () => {
  ...
  // We automatically call the login after a successful registration
  const register = async () => {
    const result = await onRegister!(email, password, firstName, lastName, userName, confirmPassword, address,isBuyer,profilePic);
    if (result && result.error) {
      alert(result.msg);
    } else {
      login();
    ...
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        ...
        <TextInput
          ...
          onChangeText={(text: string) => setProfilePic(text)}
          value={profilePic}
        />
